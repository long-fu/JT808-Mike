...
#define PASSWORD ""
#define PORT 3306
#define PACKET_TYPE 0x0200
#define ENCRYPTION 0
#define PACKET_SPLIT 0*/
...
void write_log(char *pFilename, FILE *fp, const char *pMessageinfo){
    time_t t;
    if((fp=fopen(pFilename,"a")) >=0){
        time(&t);
        fprintf(fp, "%s - ", asctime(gmtime(&t)));
...
WORD JT808_final_array(BYTE *head, BYTE *body, BYTE *finalary, WORD HeaderLength, WORD BodyLength, WORD arrlen){
    ...
    temp = combine_array(head, body, HeaderLength, BodyLength, temp);
...
WORD conn_sql(char *server, char *usr, char *pwd, char *db, int port, char *table, char *target){
    ...
    if (mysql_real_connect(&my_connection, server, usr, pwd, db, port, NULL, 0)){
        write_log("/tmp/JT808_Log", logfp, "Connect Database successfully!\n");
        sprintf(query,"select CarID,0,3,LastLat,LastLon,90,LastSpeed,LastHeading,LastGPSDate from Car where LastGPSDate>date_sub(now(),INTERVAL %d SECOND) and BookUserList like '%%%s%%' limit 100",30,target);
        //printf("QUERY:%s",query);
        t = mysql_real_query(&my_connection,query,(unsigned int) strlen(query));
        write_log("JT808_Log", logfp, query);
        if (t != 0){
        write_log("/tmp/JT808_Log", logfp, "Query Error\n");
        } else {
            write_log("/tmp/JT808_Log", logfp, "Query Succeed\n");
      ...
      write_log("/tmp/JT808_Log", logfp, "Connection failed!\n");
        mysql_close(&my_connection);
        exit(0);
        }
        return TotalNumber;
...
int SocketWrite(int nSocket, char *pBuffer, int nLen, int nTimeout){
    ...
    write_log("JT808_Log",logfp,"Packet has been sent\n");
    return nWrite;
    }
...
int conn_TCP_Server(char *ipaddr, int tcp_port){
    ...
    if ((pHost == 0)||(h_rc!=0)){
        write_log("JT808_Log", logfp, "TCP Error!\n");
        return 0;
    }
    int s = 0;
    ...
    if (s < 0){
       write_log("JT808_Log", logfp, "Socket Error!\n");
        return 0;
    }
    struct in_addr in;
    ...
    if (connect(s,(struct sockaddr *)&name,sizeof(name)) != 0){
        ...
        write_log("JT808_Log", logfp, LogMessage);
    ...
    write_log("JT808_Log", logfp, "Connection Created\n");
...
int main(void){
    ...
    {
        write_log("/tmp/JT808_Log", logfp, "Cannot Read Config File!\n");
        exit(0);
    } else {
    write_log("/tmp/JT808_Log", logfp, "Read Config File Successfully!\n");
    }
    ...
    while (1){
        ...
        header_struct = JT808_header(PacketType,bodylength,PacketEncryption,PacketSplit,dataarray[z].order,dataarray[z].phonenumber);
        ...
        //    }
           while ((SocketWrite(s,(char*)finalary,packetlength,5)) < 1){
              write_log("JT808_Log", logfp, "Disconnected...Reconnecting in 5 seconds...\n");
